...
import { LibraryIcon, ModelsIcon, RedTeamIcon, ResultsIcon, ScanIcon, ShieldIcon } from "./components/icons";
import type { ReactNode } from "react";

const NAV: { to: string; label: string; icon: ReactNode }[] = [
  { to: "/models", label: "Models", icon: <ModelsIcon /> },
  { to: "/attacks", label: "Attack Library", icon: <LibraryIcon /> },
  { to: "/run", label: "New Scan", icon: <ScanIcon /> },
  ...
  return (
    <aside className="w-64 shrink-0 border-r border-line/70 bg-panel/40 backdrop-blur-xl px-4 py-6 flex flex-col">
      ...
      <nav className="flex flex-col gap-1">
        {NAV.map((n) => (
          <NavLink key={n.to} to={n.to} className="nav-link group">
            {({ isActive }) => (
              <>
                {isActive && (
      ...
      <div className="mt-auto">
        <div className="rounded-xl border border-line/70 bg-bg2/50 p-3">
          ...
          <div className="text-[11px] text-slate-600 mt-1.5">51 attacks · OWASP LLM Top 10</div>
        </div>
      </div>
    </aside>
  );
  ...
  return (
    <header className="sticky top-0 z-10 h-14 border-b border-line/60 bg-bg/60 backdrop-blur-xl flex items-center px-8">
      <div className="text-xs text-slate-500">
        Sentinel <span className="text-slate-700">/</span>{" "}
        <span className="text-slate-300">{TITLES[key]}</span>
      </div>
      <div className="ml-auto flex items-center gap-2 text-[11px] text-slate-500">
        <span className="badge bg-accent/10 text-accent">Burp Suite for LLMs</span>
      </div>
    </header>
  );
}
